...
class UserRepository (private val url: String, private val db_user: String, private val db_password: String) {
    ...
    fun findUserById(id: Int): User? {
        ...
        DriverManager.getConnection(url, db_user, db_password).use { connection ->
            ...
            if (rs.next())
                user = User(rs.getInt("id"), rs.getString("name"), rs.getString("password"), rs.getString("email"))
        }
        return user
    }
    ...
    fun getUsers(): List<User> {
        ...
        DriverManager.getConnection(url, db_user, db_password).use { connection ->
            ...
            val rs = preparedStatement.executeQuery()
            while (rs.next()) {
                val user = User(rs.getInt("id"), rs.getString("name"), rs.getString("password"), rs.getString("email"))
                users.add(user)
            }
        }
        return users
